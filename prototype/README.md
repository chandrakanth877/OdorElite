...
A fully clickable 19-page e-commerce prototype in the OdorElite navy/gold
...
| Track | `track/?token=` or `?id=` | Timeline, tracking cards, guest claim-account |
...
| `store.js` | `OEStore`: cart, wishlist, auth, orders, addresses, cards in versioned localStorage keys (`oe.*.v1`), `oe:state` events, cross-tab sync, first-run seeds |
...
**Ava Laurent (ava@example.com)** with 5 orders in every status, 2 addresses,
and 2 cards. Sign-in accepts any email + password (labeled demo affordance).
